        ...
        <div class="border rounded-lg shadow shadow-gray-100 bg-white">
            ...
            <div v-if="editSteps.find((step) => step.id === 'VIEW_COMPANY_LIST').current === true" id="VIEW_COMPANY_LIST">
                <div class="max-h-[60vh] overflow-auto">
                    <form @submit.prevent="getCompanyListByType()" class="py-2 px-1 flex gap-2 w-[40vw]">
                        <ElementsInput :width72="true" :height11="true" :placeholder="'Search Company'" v-model="searchCompanyName" />
                        <ElementsButton :height12="true" :width32="true" :text="'Search'" :inputtype="'submit'" />
                    </form>

                    <table v-if="companyListBySearch.length > 0" class="min-w-full max-w-[50vh] min-h-[20vh]">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white align-top">
                            <tr v-for="(company, idx) in companyListBySearch" :key="idx">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ company.flexbenTypeName }} / {{ company.flexbenCampaignTitle }}</td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div v-if="$appUtil.checkPermission('FLEXBEN_SETTING_EDIT')" class="px-4 py-6 flex justify-end">
                    <ElementsButton
                        class="ml-2"
            ...
            <div v-if="editSteps.find((step) => step.id === 'CHANGE_CAMPAIGN').current === true" id="CHANGE_CAMPAIGN">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white align-top">
                            <tr v-for="(company, idx) in wantToEditList" :key="idx">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 align-baseline">
                                    <ElementsSelect :options="flexbenTypeSelectOptions" v-model="company.flexbenCampaignSeq" :isError="company.flexbenCampaignSeq === campaignSeq" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="px-4 py-6 flex justify-end">
                    <ElementsButton text="Back to List" :width28="true" :height12="true" :background-white="true" @click-event="clickSteps('VIEW_COMPANY_LIST')" />
                    <ElementsButton class="ml-2" :backgroundWhite="false" :text="'Confirm'" :width28="true" :height12="true" @click-event="goToConfirmChanges" />
                </div>
            </div>
            ...
            <div v-if="editSteps.find((step) => step.id === 'CONFIRM_CHANGE').current === true" id="CONFIRM_CHANGE">
                ...
                <div class="px-4 py-6 flex justify-end">
                    <ElementsButton text="Back to Change" :fitContent="true" :height12="true" :background-white="true" @click-event="clickSteps('CHANGE_CAMPAIGN')" />
                    <ElementsButton class="ml-2" :backgroundWhite="false" :text="'Confirm all changes'" :fitContent="true" :height12="true" @click-event="confirmAllChanges" />
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getCompanyListByType() {
            ...
            const json_query = {
                companyName: self.searchCompanyName,
                flexbenCampaignSeq: self.campaignSeq,
            };
