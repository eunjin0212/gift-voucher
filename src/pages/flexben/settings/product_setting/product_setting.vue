<template>
    <div class="mt-8 p-3 rounded-lg w-full max-w-7xl bg-white shadow-md shadow-gray-200 flex flex-col">
        <ElementsButton class="my-3" text="Update Product Status" :fit-content="true" :height12="true" @click-event="updateProductStatus()" />
        <div class="flex gap-2 max-w-full my-4">
            <ElementsSelect :width40="true" :defaultMsg="'Type'" :options="flexBenCampaignOptions" :useDefaultMsg="true" v-model="json_query.flexbenCampaignSeq" />
            <ElementsSelect :width40="true" :defaultMsg="'Status'" :options="goodsSalesStatusOptions" :useDefaultMsg="true" v-model="json_query.goodsSalesStatus" />
            <ElementsSelect :width40="true" :defaultMsg="'Select'" :options="searchOptions" :useDefaultMsg="true" v-model="json_query.searchOption" />
            <ElementsInput :width60="true" :placeholder="'Enter the keyword'" v-model="json_query.searchText" />
            <ElementsButton :width32="true" :text="'Search'" @clickEvent="getProductSettingList()" />
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-300">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="whitespace-pre-wrap px-3 py-3.5 text-center text-sm text-gray-900">
                            FlexBen <br />
                            Type
                        </th>
                        <th scope="col" class="whitespace-pre-wrap px-3 py-3.5 text-center text-sm text-gray-900">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    <tr v-for="(product, key) in productInfo.list" v-bind:key="key">
                        <td class="whitespace-pre-wrap px-3 py-4 text-center text-sm text-gray-900 sm:pl-6">{{ product.brandName }}</td>
                        <td class="whitespace-nowrap px-3 py-4 text-center text-sm text-blue-500 underline">
                            <a :href="`/flexben/settings/product_setting/detail?product=${product.flexbenGoodsSeq}`">
                                {{ product.goodsName }}
                            </a>
                        </td>
        ...
        <div class="w-full h-28 flex justify-center items-center mt-5">
            <ElementsPagination v-model="productInfo.page" :totalContent="productInfo.total" :contentsPerPage="productInfo.limit" @clickPage="clickPageButton" />
        </div>
    </div>
</template>
