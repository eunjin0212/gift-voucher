            ...
            <div class="mt-8 p-3 rounded-lg w-full max-w-7xl bg-white shadow-md shadow-gray-200 flex flex-col">
                ...
                <div class="grid grid-cols-4">
                    <div class="col-span-1">
                        <img :src="productInfo.goodsImageUrl" class="p-5" />
                    </div>
                        ...
                        <div class="space-y-12">
                            <div class="pb-6">
                                ...
                                <div class="flex gap-4 pt-5">
                                    ...
                                    <div
                                        :class="[
                                            productInfo.goodsSalesStatus === 'STOP' ? 'bg-gray-600' : productInfo.displayYn == 'N' ? 'bg-gray-200' : 'bg-indigo-600',
                                        ...
                                        <span
                                            :class="[
                                                productInfo.displayYn == 'N' ? 'translate-x-5' : 'translate-x-0',
                                                'pointer-events-none relative inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                                            ]">
                                            <span
                                                :class="[
                                                    productInfo.displayYn == 'N' ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200',
                                            ...
                                            <span
                                                :class="[
                                                    productInfo.displayYn == 'N' ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100',
                                                    'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity',
                                                ]"
                                                aria-hidden="true">
                ...
                <div class="self-end pb-5 pr-5" v-if="$appUtil.checkPermission('FLEXBEN_SETTING_EDIT')">
                    <ElementsButton :width32="true" :text="'Save'" @clickEvent="updateProductInfo" />
                </div>
            </div>
        </AppMain>
    </div>
...
export default {
    ...
    methods: {
        getProductInfo() {
            ...
            const url = self.$api('uri', 'get-flexben-product-info').replace('{flexbenGoodsSeq}', self.flexbenGoodsSeq);
        ...
        getProductSeq() {
            ...
            const flexbenGoodsSeq = urlParams.get('product');
            return flexbenGoodsSeq;
        },
        updateProductInfo() {
            const self = this;
            const url = self.$api('uri', 'put-flexben-product-info');
            const { flexbenGoodsSeq, displayYn, goodsDescription } = self.productInfo;
            const updateForm = { flexbenGoodsSeq, displayYn, goodsDescription };
            self.$axios
                .put(url, updateForm)
                .then((res) => {
    ...
    mounted() {
        const self = this;
        self.flexbenGoodsSeq = self.getProductSeq();
        self.getProductInfo();
    },
};
</script>
