        ...
        <div class="flex justify-between py-1">
            <div class="flex justify-start gap-5">
                ...
                <h1 class="text-zinc-800 text-xl font-semibold leading-10 items-center ">
                    {{isInsertMode(settingMode) ? "Update " : isEditMode(settingMode) ? "Edit" : "" }} PhilHealth
                </h1>
            </div>
                    ...
                    <div class="pb-36 mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="relative min-w-full divide-y divide-gray-300 table-fixed">
                            ...
                            <tbody class=" bg-white">
                                <tr v-for="(philHealth, index) in registerData.philHealthFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.rangeOfCompensationFrom" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.rangeOfCompensationTo" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="philHealth.moneyType" :options="moneyTypeOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.philHealthEmployee" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.philHealthEmployer" :isNumber="true" :full="true" :required="true" /></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="mt-8 p-3 rounded-lg w-full bg-white shadow-md shadow-gray-200 flex flex-col">
                ...
                <div class=" mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    <table class="relative min-w-full divide-y divide-gray-300 table-auto">
                        ...
                        <tbody class=" bg-white">
                            <tr v-for="(detail, index) in settingDetail.philHealthFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                ...
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ fitDecimalFormatWithComma(detail.philHealthEmployee) }}{{ makeMoneyTypeString(detail.moneyType) }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ fitDecimalFormatWithComma(detail.philHealthEmployer) }}{{ makeMoneyTypeString(detail.moneyType) }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ fitDecimalFormatWithComma(detail.philHealthEmployee + detail.philHealthEmployer) }}{{ makeMoneyTypeString(detail.moneyType) }}</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <form @submit.prevent="updateSetting()">
                    ...
                    <div class="pb-36 mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="relative min-w-full divide-y divide-gray-300 table-auto">
                            ...
                            <tbody class=" bg-white">
                                <tr v-for="(philHealth, index) in updateData.philHealthFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.rangeOfCompensationFrom" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.rangeOfCompensationTo" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="philHealth.moneyType" :options="moneyTypeOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="philHealth.philHealthEmployee" :isNumber="true" :full="true" :required="true" /></td>
...
export default {
    ...
    data(){
        return{
            settingType : SETTING_TYPE.PHIL_HEALTH,
    ...
    methods : {
        ...
        changeAddSettingPage(){
            ...
            .then(res => {
                ...
                self.registerData.philHealthFormList.splice(0, self.registerData.philHealthFormList.length);
                self.registerData.philHealthFormList = res.data.data.defaultValues;
        ...
        registerSetting() {
            ...
            if (self.invalidPhilHealthData(self.registerData.philHealthFormList)) {
                return;
            }
        ...
        getPayrollSettingDetail(payrollSettingSeq, settingStatus) {
            ...
            .then(res => {
                ...
                self.settingDetail.philHealthFormList = detailData.settingDetailList;
        ...
        updateSetting() {
            ...
            if (self.invalidPhilHealthData(self.updateData.philHealthFormList)) {
                return;
            }
        ...
        invalidPhilHealthData(philHealthFormList) {
            // 유효성 검증 해야함
            // from, to 는 Amount로 처리, employee, employer 는 Type에 따라 가변적으로
            const self = this;
            let isNotNumber = false;
            let invalidNumber = false;
            for (let i = 0; i < philHealthFormList.length; ++i) {
                let philHealth = philHealthFormList[i];
                let moneyType = philHealth.moneyType;

                for (let data in philHealth) {
                    ...
                    philHealth[data] = String(philHealth[data]).replaceAll(",", "");

                    if (isNaN(philHealth[data])) {
                        console.log("[invalid data] " + data + " : " + philHealth[data] + " type : " + (typeof philHealth[data]));
                        isNotNumber = true;
                        break;
                    }
                    ...
                    if (moneyType === PAYROLL_MONEY_TYPE.PERCENT && (data === "philHealthEmployee" || data === "philHealthEmployer")) {
                        max = 1000;
                    }
                    
                    philHealth[data] = Number(Number.parseFloat(philHealth[data]).toFixed(2));

                    if (self.invalidDecimal(min, max, epsilon, philHealth[data])) {
                        console.log("[invalid data] " + data + " : " + philHealth[data]);
                        invalidNumber = true;
                        break;
                    }
                }
        ...
        isPhilHealth : payrollValidator.isPhilHealth,
