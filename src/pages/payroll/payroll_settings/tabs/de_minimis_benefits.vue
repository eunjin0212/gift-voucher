    ...
    <div v-show="visible">
        <div class="flex justify-between py-1">
            <div class="flex justify-start gap-5">
                ...
                <h1 class="text-zinc-800 text-xl font-semibold leading-10 items-center ">
                    {{isInsertMode(settingMode) ? "Update " : isEditMode(settingMode) ? "Edit" : "" }} De Minimis Benefits
                </h1>
            </div>
        ...
        <div v-else-if="isInsertMode(settingMode)">
            <div class="mt-8 p-3 rounded-lg w-full bg-white shadow-md shadow-gray-200 flex flex-col">
                <form id="deMinimisBenefitsForm" @submit.prevent="registerSetting()">
                    ...
                    <div class="pb-36 mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="relative min-w-full divide-y divide-gray-300 table-fixed">
                            <thead class="sticky top-0 left-0 right-0 text-white" style="z-index: 1; background: #4031B8;">
                                <tr>
                                    <th scope="col" class="px-3 py-3 w-48 border-x border-solid border-white text-left text-sm">Name</th>
                                    <th scope="col" class="px-3 py-3 w-20 border-x border-solid border-white text-center text-sm">Unit</th>
                                    <th scope="col" class="px-3 py-3 w-16 border-x border-solid border-white text-center text-sm">Time Unit</th>
                                    <th scope="col" class="px-3 py-3 w-36 border-x border-solid border-white text-center text-sm">Ceiling</th>
                                    <th scope="col" class="px-3 py-3 w-96 border-x border-solid border-white text-center text-sm">Description</th>
                                </tr>
                            </thead>
                            <tbody class=" bg-white">
                                <tr v-for="(deMinimisBenefits, index) in registerData.deMinimisBenefitsFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><ElementsInput v-model="deMinimisBenefits.benefitName" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="deMinimisBenefits.standardUnit" :options="unitOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="deMinimisBenefits.timeUnit" :options="timeUnitOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="deMinimisBenefits.benefitCeiling" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900">
                                        <textarea
                                        form="deMinimisBenefitsForm"
                                        v-model.trim="deMinimisBenefits.benefitDescription"
                                        rows="3"
                                        required="true"
                                        :maxlength="3000"
            ...
            <div class="mt-8 p-3 rounded-lg w-full bg-white shadow-md shadow-gray-200 flex flex-col">
                ...
                <div class=" mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    <table class="relative min-w-full divide-y divide-gray-300 table-fixed">
                        ...
                        <tbody class=" bg-white">
                            <tr v-for="(detail, index) in settingDetail.deMinimisBenefitsFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-left text-sm text-gray-900"><span>{{ detail.benefitName }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ getDisplayTextOfStandardUnit(detail.standardUnit) }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ getDisplayTextOfTimeUnit(detail.timeUnit) }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ isAmount(detail.standardUnit) ? fitDecimalFormatWithComma(detail.benefitCeiling) : detail.benefitCeiling }}{{ makeStandardUnitString(detail.standardUnit) }}</span></td>
                                <td class="px-3 py-3 border-x border-solid border-gray-200 text-center text-sm text-gray-900"><span>{{ detail.benefitDescription }}</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div v-else-if="isEditMode(settingMode)">
            <div class="my-8 p-3 rounded-lg w-full bg-white shadow-md shadow-gray-200 flex flex-col">
                <form id="deMinimisBenefitsForm" @submit.prevent="updateSetting()">
                    <div class="my-2 w-full flex flex-col gap-8" >
                        <div class="flex justify-start gap-2">
                            <span class="mt-2 w-48">Title*</span>
                            <ElementsInput
                    ...
                    <div class="pb-36 mt-6 overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="relative min-w-full divide-y divide-gray-300 table-auto">
                            ...
                            <tbody class=" bg-white">
                                <tr v-for="(deMinimisBenefits, index) in updateData.deMinimisBenefitsFormList" v-bind:key="index" class="border-x border-y border-solid border-gray-200">
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="deMinimisBenefits.benefitName" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="deMinimisBenefits.standardUnit" :options="unitOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsSelect v-model="deMinimisBenefits.timeUnit" :options="timeUnitOption" :full="true" class="mt-[16px] h-[48px] rounded" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900"><ElementsInput v-model="deMinimisBenefits.benefitCeiling" :isNumber="true" :full="true" :required="true" /></td>
                                    <td class="px-3 py-4 border-x border-solid border-gray-200 text-sm text-gray-900">
...
export default {
    ...
    data(){
        return{
            settingType : SETTING_TYPE.DE_MINIMIS_BENEFITS,
            ...
            unitOption : STANDARD_UNIT_OPTION,
            timeUnitOption: TIME_UNIT_OPTION,
    ...
    methods : {
        ...
        changeAddSettingPage(){
            ...
            .then(res => {
                ...
                self.registerData.deMinimisBenefitsFormList.splice(0, self.registerData.deMinimisBenefitsFormList.length);
                self.registerData.deMinimisBenefitsFormList = res.data.data.defaultValues;
        ...
        registerSetting() {
            ...
            if (self.invalidDeMinimisBenefitsData(self.registerData.deMinimisBenefitsFormList)) {
                return;
            }
        ...
        getPayrollSettingDetail(payrollSettingSeq, settingStatus) {
            ...
            .then(res => {
                ...
                self.settingDetail.deMinimisBenefitsFormList = detailData.settingDetailList;
        ...
        updateSetting() {
            ...
            if (self.invalidDeMinimisBenefitsData(self.updateData.deMinimisBenefitsFormList)) {
                return;
            }
        ...
        isAmount(value) {
            return value === PAYROLL_STANDARD_UNIT.AMOUNT;
        },
        invalidDeMinimisBenefitsData(deMinimisBenefitsFormList) {
            const self = this;
            let isNotNumber = false;
            let invalidNumber = false;

            for (let i = 0; i < deMinimisBenefitsFormList.length; ++i) {
                let deMinimisBenefits = deMinimisBenefitsFormList[i];
                ...
                if (self.invalidTimeUnitOption(timeUnit)) {
                    return true;
                }

                
                deMinimisBenefits.benefitCeiling = String(deMinimisBenefits.benefitCeiling).replaceAll(",", "");
                // ceiling 만 남게 된다.
                // 숫자가 아닌 경우 실패
                if (isNaN(deMinimisBenefits.benefitCeiling)) {
                    console.log("[invalid data] benefitCeiling : " + deMinimisBenefits.benefitCeiling);
                    isNotNumber = true;
                    break;
                }
                ...
                if (standardUnit === PAYROLL_STANDARD_UNIT.AMOUNT) {
                    // 소수점 3째 자리에서 반올림
                    deMinimisBenefits.benefitCeiling = Number(Number.parseFloat(deMinimisBenefits.benefitCeiling).toFixed(2));
                } else if (standardUnit === PAYROLL_STANDARD_UNIT.PERCENT) {
                    // 소수점 3째 자리에서 반올림
                    deMinimisBenefits.benefitCeiling = Number(Number.parseFloat(deMinimisBenefits.benefitCeiling).toFixed(2));
                    max = 1000;
                } else if (standardUnit === PAYROLL_STANDARD_UNIT.DAY) {
                    // 소수점 1째 자리에서 반올림
                    deMinimisBenefits.benefitCeiling = Number(Number.parseFloat(deMinimisBenefits.benefitCeiling).toFixed(0));
                    min = 0;
                    max = 366;
                }


                if (self.invalidDecimal(min, max, epsilon, deMinimisBenefits.benefitCeiling)) {
                    console.log("[invalid data] benefitCeiling : " + deMinimisBenefits.benefitCeiling);
                    invalidNumber = true;
                    break;
                }
            ...
            if (invalidNumber) {
                alert("Amount must be 0 or more and less than 10,000,000,000,000.(0.01 ~ 10,000,000,000,000) \nPercent must be 0 or more and less than 1000.(0 ~ 1000)\nDay must be greater than 0 and less than 366.\n");
                return true;
            }

            return false;
        },
        getDisplayTextOfStandardUnit(standardUnit) {
            let result = "";
            switch(standardUnit) {
                ...
                case PAYROLL_STANDARD_UNIT.PERCENT:
                    result = "Percent";
                    break;
                case PAYROLL_STANDARD_UNIT.DAY:
                    result = "Day";
                    break;
                default:
                    break;
        ...
        getDisplayTextOfTimeUnit(timeUnit) {
            let result = "";
            switch(timeUnit) {
                case PAYROLL_TIME_UNIT.PER_YEAR:
                    result = "Per Year";
                    break;
                case PAYROLL_TIME_UNIT.PER_MONTH:
                    result = "Per Month";
                    break;
                case PAYROLL_TIME_UNIT.PER_DAY:
                    result = "Per Day";
                    break;
                default:
                    break;
    ...
    watch: {
        tabName: function(newValue) {
            ...
            if (self.isDeMinimisBenefits(newValue)) {
                self.getSettingList();
            }
        }
    },
