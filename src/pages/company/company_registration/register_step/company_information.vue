        ...
        <form @submit.prevent="clickSubmit">
            <div class="my-2 flex flex-col gap-4 mx-4">
                ...
                <div class="w-full">
                    ...
                    <input
                        :value="registerData.companyIdNumber"
                ...
                <ElementsInput :name="'Company Name'" :full="true" :maxlength="60" v-model="registerData.companyName" :required="true" />
                <ElementsInput :name="'Representative Name'" :full="true" :maxlength="60" v-model="registerData.representativeName" :required="true" />
                <ElementsInput :name="'Business Registration Number'" :full="true" :maxlength="60" v-model="registerData.businessRegistrationNumber" :required="true" />
                ...
                <div class="w-full flex gap-2">
                    ...
                    <ElementsInput class="grow" v-model="registerData.contactNumber" :full="true" :maxlength="200" :required="true" :inputtype="'tel'" />
                </div>
                <ElementsInput v-model="registerData.contactEmail" :name="'Company Email'" :full="true" :inputtype="'email'" :maxlength="100" :required="true" />
                <ElementsInput v-model="registerData.companyAddress" :name="'Company Address'" :full="true" :maxlength="199" :required="true" />
                ...
                <div>
                    ...
                    <template v-if="contractFile">
                        <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 flex justify-between">
                            ...
                            <div class="border border-red-600 p-2 bg-white rounded-md font-semibold text-red-600 cursor-pointer" name="poDocumentFile" @click="$emit('update:contractFile', null)">
                                delete
                            </div>
                        </dd>
                    </template>
                    ...
                    <template v-else>
                        <dd class="text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                            <input type="file" class="w-full mt-1 shadow-sm block sm:text-sm bg-white border-gray-300 rounded-md" multiple @input="afterFileSelect" :required="!noNeedDocs" />
                        </dd>
                    </template>
                </div>
                ...
                <ElementsInput v-model="registerData.subscriptionPicName" :name="'PIC Name'" :full="true" :maxlength="60" :required="true" />
                <ElementsInput v-model="registerData.subscriptionPicDepartment" :name="'PIC Department'" :full="true" :maxlength="60" :required="true" />
                <ElementsInput v-model="registerData.subscriptionPicEmail" :name="'PIC Email'" :full="true" :inputtype="'email'" :maxlength="100" :required="true" />
                <div class="text-sm font-semibold text-slate-800">PIC Contact Number</div>
                <div class="w-full gap-2v flex gap-1">
                    <ElementsInput :modelValue="63" :disabled="true" :width14="true" :maxlength="200" />
                    <ElementsInput class="grow" v-model="registerData.subscriptionPicPhoneNumber" :full="true" :maxlength="60" :required="true" :inputtype="'tel'" />
                </div>
            </div>

            <div class="flex justify-end gap-4 my-3">
                <ElementsButton :backgroundWhite="true" :width32="true" :text="'Cancel'" :inputtype="'button'" @clickEvent="$emit('back-to-list')" />
                <ElementsButton :width32="true" :text="'Next'" :inputtype="'submit'" />
            </div>
        </form>
    </div>
</template>
    ...
    methods: {
        getInquiryData() {
            ...
            self.$axios
                .get(`${url}/${inquirySeq}`)
                .then((res) => {
                    const { inquiryCompanyName, picDepartmentName, picEmail, picName, picPhoneNumber } = res.data.data.data;
                    self.registerData = {
                        ...self.registerData,
