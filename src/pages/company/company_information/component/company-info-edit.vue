        ...
        <form @submit.prevent="clickSubmitButton">
            <div class="flex flex-col gap-4">
                ...
                <div class="w-full flex gap-2">
                    ...
                    <ElementsInput class="grow" v-model="editCompanyData.contactNumber" :full="true" :maxlength="200" :required="true" :inputtype="'tel'" :disabled="!isEdit" />
                </div>
                <ElementsInput v-model="editCompanyData.contactEmail" :name="'Company Email'" :full="true" :inputtype="'email'" :maxlength="100" :required="true" :disabled="!isEdit" />
                <ElementsInput v-model="editCompanyData.companyAddress" :name="'Company Address'" :full="true" :maxlength="199" :required="true" :disabled="!isEdit" />
                <div>
                    ...
                    <template v-if="editCompanyData.contractFilePath">
                        <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 flex justify-between">
                            ...
                            <div v-if="isEdit" class="border border-red-600 p-2 bg-white rounded-md font-semibold text-red-600 cursor-pointer" @click="deleteContractFilePath">delete</div>
                        </dd>
                    </template>
                    <template v-else-if="contractFile.name">
                        <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 flex justify-between">
                            ...
                            <div v-if="isEdit" class="border border-red-600 p-2 bg-white rounded-md font-semibold text-red-600 cursor-pointer" @click="deleteContractFile">delete</div>
                        </dd>
                    </template>
            ...
            <div class="flex justify-end gap-4 my-3" v-if="isEdit">
                <ElementsButton :width32="true" :text="'Save'" :inputtype="'submit'" v-if="$appUtil.checkPermission('COMPANY_SETTING_EDIT')" />
            </div>
        </form>
    </div>
</template>
...
export default {
    ...
    mounted() {
        ...
        const {
            ...
            contactEmail,
            companyAddress,
            subscriptionPicDepartment,
            subscriptionPicName,
            subscriptionPicEmail,
            subscriptionPicPhoneNumber,
        } = self.registerData;
    ...
    methods: {
        ...
        clickSubmitButton() {
            ...
            if (self.contractFile.name) {
                ...
                if (self.registerData.contractFilePath) {
